...
def main(session):
    vm = session.xenapi.VM.get_by_name_label('Ubuntu12.04-II')[0]
    for i in range(2):  
      cloned = session.xenapi.VM.clone(vm, ("newtwo-%d" % i))
      
    print "Starting VM"
    for i in range(2): 
      session.xenapi.VM.start(("newtwo-%d" % i), False, True)
    print "  VM is booting"

    #while task.task_status_type == "pending":
